...
configure_address() {
    ...
    # Fetch data
    DATA=$(curl -s "http://$ADDRESS:19998/info" || echo "error")
...
EOF
    ...
    # Check if smartd metrics are available and update prometheus.conf
    if [[ -n "$SMARTD_DATA" && "$SMARTD_DATA" != "error" ]]; then
        # Check if the job for the address already exists
        if grep -Fq "url: 'http://$ADDRESS:19997/metrics'" "$PROMETHEUS_CONF_PATH"; then
            read -p "Smartd metrics configuration for $ADDRESS already exists in prometheus.conf. Overwrite? [y/N]: " choice
            if [[ "$choice" =~ ^[Yy]$ ]]; then
                echo "Replacing existing smartd configuration for $ADDRESS in prometheus.conf."
                sed -i "/  - name: smartd_$HOSTNAME/,+3d" "$PROMETHEUS_CONF_PATH"
            else
                echo "Skipping prometheus.conf update."
                return 1
            fi
        fi
        if ! grep -Pq '^\s*jobs:' "$PROMETHEUS_CONF_PATH" || grep -Pq '^\s*#.*jobs:' "$PROMETHEUS_CONF_PATH"; then
            echo "jobs:" >> "$PROMETHEUS_CONF_PATH"
        fi
        echo "Appending new smartd configuration for $ADDRESS in prometheus.conf."
        echo "  - name: smartd_$HOSTNAME" >> "$PROMETHEUS_CONF_PATH"
        echo "    app: SMART" >> "$PROMETHEUS_CONF_PATH"
        echo "    vnode: $HOSTNAME" >> "$PROMETHEUS_CONF_PATH"
        echo "    url: 'http://$ADDRESS:19997/metrics'" >> "$PROMETHEUS_CONF_PATH"
    else
        echo "No smartd metrics available for $ADDRESS or not reachable. Skipping prometheus.conf update."
    fi
...
# Handle a list of addresses or a single address
if [[ -n "$1" && -f "$1" ]]; then
    error_count=0
    total_count=0
